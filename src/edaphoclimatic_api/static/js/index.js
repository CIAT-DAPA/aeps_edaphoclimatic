...
var layerControl = L.control
  .layers(overLay, { "Zona de Estudio": capZonaEstudio })
  .addTo(map);
...
function chartBar(data, categories) {
  ...
  var chart = new ApexCharts(
    document.querySelector("#chart-relaciones"),
    options
  );
  chart.render();
}
...
function pdf() {
  ...
  doc.text(
    docWidth - 10,
    currentHeight,
    "https://twitter.com/boyacaseadaptac",
    "right"
  );
  ...
  doc.text(
    docWidth - 10,
    currentHeight,
    "https://www.facebook.com/BoyacaSeAdaptaC",
    "right"
  );
    ...
    const cluster = datos["cluster"];
      if (cluster.message) {
        ...
        let lines = doc.splitTextToSize(
          `Usted se encuentra dentro de la zona edafoclimática ${cluster.numCluster}.`,
          docWidth - 20
        );
        doc.text(lines, 10, currentHeight);
        currentHeight += pdfConfig.subLineHeight;
        ...
        for (const variable in cluster) {
          if (variable !== "numCluster") {
            ...
            doc.cell(
              ...
              currentHeight,
              docWidth / 3,
              pdfConfig.cell * count,
              `${infodatos[variable].nombre} ${
                infodatos[variable].unidad &&
                "(" + infodatos[variable].unidad + ")"
              }`
            );
...
function render() {
  ...
  if (map.hasLayer(capCluster)) {
    ...
    const accordionBody = document.getElementById(
      `accordion-body-${categoria}`
    );

    // Renderizado tabla de zonifiacion Edafoclimatica

    console.log("respuesta clusters", datos["cluster"]);
    if (datos["cluster"].message) {
      ...
      for (const variable in cluster) {
        if (variable !== "numCluster") {
          let num = 1;
          const count = Object.keys(cluster[variable]).length;
          tabla += `<tr><td rowspan="${count}" class="align-middle">${
            infodatos[variable].nombre
          } ${
            infodatos[variable].unidad && "(" + infodatos[variable].unidad + ")"
          }</td>`;
          if (Object.hasOwnProperty.call(cluster, variable)) {
            const datosVariable = cluster[variable];
            for (const rango in datosVariable) {
      ...
      if (categoria !== "zonas") {
        ...
        variables.forEach((key) => {
          ...
          accordionBody.insertAdjacentHTML(
            "beforeend",
            `<div class="card my-3">
                    <h5 class="card-header"> ${nombre} ${
              sigla && " - " + sigla
            } ${unidad && " (" + unidad + ")"}
                      ${
                        descripcion &&
                        '<a class="d-inline-block" data-bs-toggle="tooltip" data-bs-html="true" title="' +
                          descripcion +
                          '">' +
        ...
        if (categoria == "textura_suelo") {
          const textura = infodatos.categorias.textura_suelo.variables.filter(
            (variable) => variable != "textura"
          );
          const series = textura.map((element) => datos[element]);
          const labels = textura.map((element) => infodatos[element].nombre);
          accordionBody.insertAdjacentHTML(
            "beforeend",
            '<div id="chart-textura"> </div>'
          );
          chartRadialBar(series, labels, datos["simboloTextu"]);
        } else if (categoria == "relaciones") {
          const relaciones = infodatos.categorias.relaciones;
          const data = relaciones.variables.map((element) => datos[element]);
          const categories = relaciones.variables.map((element) =>
            infodatos[element].nombre.replace("Relación catiónica ", "")
          );
          accordionBody.insertAdjacentHTML(
            "beforeend",
            '<div id="chart-relaciones"> </div>'
          );
